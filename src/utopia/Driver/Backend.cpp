...
bool Backend::process(llvm::Module *mod, BackendContext &backendCtx,
  ...
  if (!target) {
    std::cerr << "[Backend Error] Target lookup failed: " << error << "\n";
    return false;
  }
...

  {
    ...
    if (options.emitLLVM) {
      std::error_code ec;
      llvm::raw_fd_ostream destLL(outBasePath + ".ll", ec,
                                  llvm::sys::fs::OF_None);
      if (!ec)
    ...
    if (options.emitAsm) {
      std::error_code ec;
      llvm::raw_fd_ostream destAsm(outBasePath + ".s", ec,
                                   llvm::sys::fs::OF_None);
      ...
      tmAsm->addPassesToEmitFile(passAsm, destAsm, nullptr,
    ...
    targetMachine->addPassesToEmitFile(passObj, destObj, nullptr,
